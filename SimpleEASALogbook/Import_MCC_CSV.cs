...
namespace SimpleEASALogbook
{
    class Import_MCC_CSV
    {
        ...
        Flight CreateFlightMCCv4(string[] csvline)
        {
            ...
            if (!TimeSpan.TryParse(csvline[7], out Endtime))
            {
            ...
            if (!int.TryParse(csvline[42], out DayLanding) || csvline[39].Contains("TRUE"))
            {
                DayLanding = 0;
            }

            if (!int.TryParse(csvline[43], out NightLanding) || csvline[39].Contains("TRUE"))
            {
                NightLanding = 0;
            }

            if (!DateTime.TryParse(csvline[0], out date_of_sim) || !csvline[2].Contains("TRUE"))
            {
                date_of_sim = DateTime.MinValue;
            }
            if (csvline[26].Length > 0 && !csvline[2].Contains("TRUE"))
            {
            ...
            {
                return new Flight(StartDate, FROM, Starttime, TO, Endtime, Type, Aircraft, TimeSpan.Zero, TimeSpan.Zero, MultiPilotTime, TotalTimeOfFlight, PIC, DayLanding, NightLanding, NightTime, IFRTime, PICTime, CopilotTime, DualTime, InstructorTime, date_of_sim, Type_of_sim, sim_time, remarks, nextpageafter);
            }
        }
